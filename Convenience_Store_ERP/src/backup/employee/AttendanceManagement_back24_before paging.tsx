...
function AttendanceManagement() {
  ...
  const fetchAttendanceData = async () => {
    ...
    try {
      ...
      // Supabase에서 출결 데이터 조회 (attendance 테이블만 조회)
      ...
      if (employees.length === 0) {
        ...
        // 직원 데이터가 있는 경우, 각 직원별로 출결 데이터 매칭 (DB 데이터만 사용)
        const processedAttendances: Attendance[] = employees.map(emp => {
          console.log(`${emp.employee_name} 출결 데이터 확인 중...`);
          ...
          if (empAttendance) {
            ...
            // 출결 데이터가 없는 경우 (결근)
            console.log(`${emp.employee_name} - 출결 데이터 없음, 결근 처리`);
            return {
              employee_no: emp.employee_no,
              employee_name: emp.employee_name,
              attendance_date: selectedDate,
        ...
        console.log('최종 처리된 출결 데이터:', processedAttendances);
