...
<script>
	$(function(){
		...
		$('#profileFrm').click(function(){
			var file = $('#profileFrm input[type="file"]').val();
			console.log(file);
			
			if (file != '') {
				$('#profileFrm button').attr('type','submit');
				$('#profileFrm').submit();
			}
		});
			...
			<ul class="mb-profile">
				<li>
					<div class="profileImage">
						<c:if test ="${profile.profile_thumbnail != null}">
						<img id="img" src="/libraryUploadImg/${profile.profile_thumbnail }">
						</c:if>
						<c:if test ="${profile.profile_thumbnail == null}">
						<img id="img" src="/resources/css/admin/imgs/account.png">
						</c:if>
						<div class="bigImg">
							<c:if test ="${profile.profile_thumbnail != null}">
							<img src="/libraryUploadImg/${profile.profile_thumbnail }">
							</c:if>
							<c:if test ="${profile.profile_thumbnail == null}">
				...
				<li>
					<form id="profileFrm" action="/admin/member/profile/registerProfile" method="post" enctype="multipart/form-data">
						<input type="hidden" name="member_id" value="${member.member_id} ">						
						<input type="file" name="file" id="file">
						<button type="button" class="btn btn-default">프로필 이미지 등록</button>
					</form>
				</li>
			</ul>
