...
<link rel="stylesheet" href="/resources/css/admin/mylibrary/reservation_book_list.css">
<script>	
	$(function(){
		$('#searchBtn').click(function(){
			...
			if (keyword == '') {
				location.href = "/admin/member/mylibrary/reservation_book_list?num=1&member_id="+member_id;
			} else {
				location.href = "/admin/member/mylibrary/reservation_book_list?num=1&member_id="+member_id+"&searchType="+searchType+"&keyword="+keyword;
			}
		});
	})
		...
		<div class="mylibrary-container">
			...
			<div class="mylibrary-wrap">
				...
				<ul class="mylibrary-menu-container">
					...
					<li><a href="/admin/member/mylibrary/borrow_current_list?num=1&member_id=${member.member_id }">대출 이력</a></li>
				</ul>
				...
				<ul  class="search-keyword">
					<li>
						<select id="searchType" name="searchType">
							...
							<option value="reservation_date">예약일</option>
				...
				<div class="mylibrary-borrow-wrap">
					<c:if test="${ empty reservation_book_list }">
					<p class="null_list_msg">예약한 도서가 없습니다.</p>
					</c:if>
				
					<c:if test="${ !empty reservation_book_list }">
					<c:forEach items="${reservation_book_list}"  var="reservation_book_list" varStatus="status">
					<div class="borrow-list-container book-list-container-${status.index}">
						...
						<div class="book-thumbnail">
							<a href=""><img src="/libraryUploadImg/${reservation_book_list.book_thumbnail}"></a>
						</div>
					
						<ul class="borrow-list">
							<li><p>도서명 : ${reservation_book_list.book_title}</p></li>
							<li><p>자료실 위치 : ${reservation_book_list.book_position}</p></li>
							<li>
								<p>
									<span>예약일 : 
									<fmt:formatDate value="${reservation_book_list.reservation_date}"  pattern="yyyy. MM. dd" />
									</span>
									</p>
								</li>
							<li>
								<p>
									<span>대출 상태 : ${reservation_book_list.borrow_state}</span>
									<span> | </span>
									<span>반납 예정일 : ${reservation_book_list.borrow_end}</span>
								</p>
							</li>
						</ul>
					</div>
