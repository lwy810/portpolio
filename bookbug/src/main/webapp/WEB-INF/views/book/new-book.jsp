...
<script>
	$(function(){
		...
		$('.book-search-bar li:nth-child(4)').css('background','#fdf4d1')
		...
		$('#searchBtn').click(function(){
			...
			location.href="/book/new-book?num="+num+"&book_category_type="+book_category_type+"&registerPeriod="+registerPeriod+"&searchType="+searchType+"&keyword="+keyword;
		...
		<div class="book-container">
			...
			<div class="book-search-wrap">
				...
				<ul  class="search-keyword search-keyword-booklist keyword-search">
					...
					<li><input type="search" id="keyword" name="keyword"  placeholder="검색어 입력"  class="input-frm keyword"></li>
				...
				<div class="book-list-wrap">
					<c:if test="${ empty new_book_list }">
					<p class="null_list_msg">검색된 도서가 없습니다.</p>
					</c:if>
				
					<c:if test="${ !empty new_book_list }">
					<c:forEach items="${new_book_list}"  var="new_book_list" varStatus="status">
					<div class="book-list-container book-list-container-${status.index}">
						...
						<div class="book-thumbnail">
							<a href=""><img src="/libraryUploadImg/${new_book_list.book_thumbnail}"></a>
						</div>
					
						<ul class="book-list">
							<li><a href="/book/view?book_id=${new_book_list.book_id}">${new_book_list.book_title}</a></li>
							<li><p><span>저자 : ${new_book_list.book_author}</span><span>|</span><span>발행자 : ${new_book_list.book_publisher}</span><span>|</span><span>발행일 : ${new_book_list.book_publisher_year}</span></p></li>
							<li><p><span>등록번호 : ${new_book_list.book_id}</span><span>|</span><span>ISBN : ${new_book_list.book_isbn}</span><span>|</span><span>청구기호 : ${new_book_list.book_callnumber}</span></li>
							<li><p><span>자료위치 : ${new_book_list.book_position}</span><span>|</span><span>대출 횟수 : ${new_book_list.book_borrow_cnt}회</span></p></li>
							<li>
								<c:if test = "${new_book_list.book_rental_able eq '대출 가능'}">
								<div><p class="rental-enable">${new_book_list.book_rental_able}[비치중]</p></div>
								</c:if>
								<c:if test = "${new_book_list.book_rental_able eq '대출 불가'}">
								<div><p class="rental-unable">${new_book_list.book_rental_able}[대출중]</p></div>
								</c:if>
								...
								<div class="reserve-container">
									...
									<input type="hidden" name="book_id" value="${new_book_list.book_id}">
				...
				<div class="pagenation-wrap">
					<ul class="pagenation">
						<%-- 이전(prev) 버튼 --%>
						<c:if test ="${page.prev}">
						<li><a href="/book/new-book?num=${page.startPageNum - 1}${page.searchTypeKeyword}" class="prev">이전</a></li>
						</c:if>
						...
						<li>
							...
							<a class ="a"  href="/book/new-book?num=${num}${page.searchTypeKeyword}">${num}</a>
							</c:if>
							<c:if test="${selectNum == num}">
						...
						<li><a href="/book/new-book?num=${page.endPageNum + 1}${page.searchTypeKeyword}" class="next">다음</a></li>
