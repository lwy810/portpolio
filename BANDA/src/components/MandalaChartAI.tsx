...
export const MandalaChartAI: React.FC = () => {
  ...
  const convertApiDataToMandalaData = (data: MandalartData, mandalaChartId: string): MandalaData => {
    const sections = data.coreGoals?.slice(0, 4).map((coreGoal, CoreIndex) => ({
      core_goal_id: CoreIndex, // 이미 1부터 시작하는 값 사용
      ...
      subGoals: coreGoal.subGoals?.slice(0, 5).map((subGoal, subIndex) => ({
        core_goal_id: coreGoal.core_goal_id, // 이미 1부터 시작하는 값 사용
        sub_goal_id: subIndex, // 이미 1부터 시작하는 값 사용
  ...
  const mandalaChartId = `${currentUser?.user_email}_mandala_${(currentUser?.user_mandala_charts_count || 0) + 1}`;
    ...
    // core_goals: 복합키 생성을 위해 인덱스 정보만 전달 (DAO에서 실제 복합키 생성)
    const coreGoals: CoreGoalInsert[] = parsedData.coreGoals.map((coreGoal: CoreGoalProps, index: number) => ({
      mandala_chart_id: mandalaChartId,
      core_goal_id: `${index + 1}`, // 임시값, DAO에서 실제 복합키 생성
      core_goal_title: coreGoal.core_goal_title || ''
    }));

    // sub_goals: 플랫 구조로 변환 (DAO에서 복합키 생성)
    ...
    parsedData.coreGoals.forEach((coreGoal: CoreGoalProps, coreIndex: number) => {
      if (coreGoal.subGoals && coreGoal.subGoals.length > 0) {
        coreGoal.subGoals.forEach((subGoal: SubGoalProps, subIndex: number) => {
          subGoals.push({
            sub_goal_id: `${coreIndex + 1}_${subIndex + 1}`, // 임시값, DAO에서 실제 복합키 생성
            core_goal_id: `${coreIndex + 1}`, // 임시값, DAO에서 실제 복합키 생성
            sub_goal_title: subGoal.sub_goal_title || ''
          });
        });
      }
