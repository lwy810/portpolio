...
export const MandalaChart: React.FC = () => {
  ...
  const [selectedCore, setSelectedCore] = useState<number | null>(null); // number -> string으로 변경
  ...
  const convertApiDataToMandalaData = (data: MandalartData, mandalaChartId: string): MandalaData => {
    const sections = data.coreGoals?.slice(0, 4).map((coreGoal, CoreIndex) => ({
      core_goal_id: CoreIndex, // 이미 string 형태의 복합 ID 사용
      ...
      subGoals: coreGoal.subGoals?.slice(0, 5).map((subGoal, subIndex) => ({
        core_goal_id: coreGoal.core_goal_id, // 이미 string 형태의 복합 ID 사용
        sub_goal_id: subIndex, // 이미 string 형태의 복합 ID 사용
  ...
  const fetchMandalartPlan = async () => {
    ...
    try {
      ...
      // API 응답 데이터 구조 변환 - string 형태의 복합 ID 사용
      const transformedData: MandalartData = {
        ...
        coreGoals: parsedData.coreGoals?.map((coreGoal: any, index: number) => ({
          ...
          subGoals: coreGoal.subGoals?.map((subGoal: any, subIndex: number) => ({
            core_goal_id: `${mandalaChartId}_core_goal_${index + 1}`, // 복합 ID 형태
            sub_goal_id: `${mandalaChartId}_core_goal_${index + 1}_sub_goal_${subIndex + 1}`, // 복합 ID 형태
            ...
            {Array.from({ length: 25 }, (_, index) => {
              ...
              if (row === 2 && col === 2) {
                return (
                  <Card
                    key={index}
                    onClick={() => setSelectedCore(section?.core_goal_id)}
                    onMouseEnter={handleMouseEnter}
                    onMouseLeave={handleMouseLeave}
                    className="mandala-cell mandala-core"
                  >
                    <div className="text">{section?.core_goal_title}</div>
                  </Card>
                );
              }
