...
export const MandalaChart: React.FC = () => {
  ...
  const loadExistingMandalaData = useCallback(async () => {
    ...
    try {
      ...
      console.log('🔍 기존 만다라 데이터 조회 시작');
      
      // 가장 최신 만다라 차트 조회
      const existingData = await MandalaDAO.getLatestMandalaChart(currentUser.user_email);
      
      if (existingData) {
        console.log('📋 기존 데이터 발견:', existingData);
        
        // DB 데이터를 MandalartData 형식으로 변환
        ...
        setIsSaved(true); // 이미 저장된 데이터이므로 저장 완료 상태로 설정
        
        console.log('✅ 기존 만다라 데이터 로드 완료');
      } else {
        console.log('📝 기존 데이터 없음 - 새로 생성 가능');
      }
    } catch (error) {
      console.error('❌ 기존 데이터 로드 실패:', error);
      setError(`기존 데이터 로드 중 오류가 발생했습니다: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
...
>>>>>>> 89af60585cdb45c63f4aa665e207511a9695a5c4
          ...
          <div className="mandala-input-group">
            ...
            >
              {isLoading ? '생성 중...' : isLoadingFromDB ? '로딩 중...' : 'AI로 채우기 🤖'}
            </Button>
          </div>
