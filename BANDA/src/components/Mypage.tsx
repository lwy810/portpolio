...
<<<<<<< HEAD
  // 공통: 현재 로그인 사용자 ID - 수정된 버전
  const getUserId = async () => {
    console.log("currentUser:", currentUser);
    
    // 먼저 currentUser에서 이메일 가져오기
    ...
    // currentUser가 없으면 supabase auth에서 가져오기
    const { data } = await supabase.auth.getUser();
    const authEmail = data.user?.email;
    console.log("authEmail from supabase auth:", authEmail);
  ...
  useEffect(() => {
    const loadProfileAndAttendance = async () => {
      ...
      console.log("loadProfileAndAttendance - userId:", userId);
      if (!userId) return;
      ...
      console.log("출석 데이터:", att);
      console.log("출석 오류:", aErr);
      

      if (!aErr && att && att.length > 0) {
        setAttendanceCount(att[0].consecutive_count);
        console.log("출석 일수:", att[0].consecutive_count)
      } else {
  ...
  useEffect(() => {
    const loadGoals = async () => {
      try {
        ...
        console.log("만다라 데이터:", mandala);
        console.log("만다라 오류:", mErr);

        if (mErr || !mandala) {
          console.log("만다라 차트를 찾을 수 없습니다.");
          return;
        }
        ...
        console.log("코어 목표들 데이터:", cores);
        console.log("코어 목표들 오류:", cErr);
        
        if (cErr || !cores || cores.length === 0) {
          console.log("코어 목표들을 찾을 수 없습니다.");
          return;
        }
        ...
        for (const cg of cores) {
          ...
          console.log(`${cg.core_goal_title}의 서브 목표들:`, subs);
          if (subErr) console.log(`서브 목표 조회 오류:`, subErr);
          ...
          console.log(`추가할 목표 데이터:`, goalData);
  ...
  const handleAttendance = async () => {
    ...
    try {
      const userId = await getUserId();
      console.log("handleAttendance - userId:", userId);
      if (!userId) {
        console.log("userId가 없어서 출석 체크 중단");
        return;
      }
      ...
      const { data: todayAttendance, error: todayErr } = await supabase
        ...
        .select("*")
        .eq("user_email", userId)
        .gte("attendance_created_at", `${todayStr}T00:00:00`)
        .lte("attendance_created_at", `${todayStr}T23:59:59`)
        .maybeSingle();

      console.log("오늘 출석 데이터:", todayAttendance);
      console.log("오늘 출석 조회 오류:", todayErr);

      if (todayAttendance && !todayErr) {
      ...
      console.log("현재 프로필 데이터:", currentData);
      if (selectError) {
      ...
      console.log("현재 연속일수:", currentCountConsecutiveDays_);
      ...
      const { data: yesterday, error: yesterdayErr } = await supabase
        .from("attendance_logs")
        .select("attendance_created_at")
        .eq("user_email", userId)
        .gte("attendance_created_at", `${ymd}T00:00:00`)
        .lte("attendance_created_at", `${ymd}T23:59:59`)
        .maybeSingle();

      console.log(`yesterday: ${JSON.stringify(yesterday)}`);
      console.log(`yesterday error: ${JSON.stringify(yesterdayErr)}`);
      
      const newCount = yesterday?.attendance_created_at ? currentCountConsecutiveDays_ + 1 : 1;
      ...
      const { error: insErr } = await supabase
        ...
        .insert([
          {
            user_email: userId,
            attendance_created_at: today.toISOString()
          },
        ]);

      if (insErr) {
        console.error("출석 기록 저장 오류:", insErr);
        return;
      }
      ...
      const { error: exUpdateErr } = await supabase
        ...
        .update({
          ...
          user_consecutive_days: newCount
        })
        .eq("user_email", userId);
        
      if (exUpdateErr) {
        console.error("프로필 업데이트 오류:", exUpdateErr);
        throw exUpdateErr;
      }

      // 별 로그도 기록 (XP 트리거 발동)
      const { error: starErr } = await supabase
        .from("star_earned_logs")
        .insert([
          {
            user_email: userId,
            star_earned_count: 1,
      ...
      if (starErr) {
        console.error("별 로그 저장 오류:", starErr);
      }
      ...
      console.error("출석 체크 중 전체 오류:", error);
      // 오류가 발생해도 사용자에게 피드백 제공
      setShowAttendancePopup(true);
      setTimeout(() => setShowAttendancePopup(false), 1600);
    }
    console.log("=== 출석 체크 종료 ===");
  };
    ...
    const fetchOrCreateQuote = async () => {
      ...
      try {
        ...
        try {
          ...
          // API 키 오류나 기타 OpenAI 오류 시 기본 명언 사용
          const fallbackQuotes = [
            { quote_text: "성공은 준비된 자에게만 찾아온다", author: "루이 파스퇴르" },
            { quote_text: "어제의 나보다 나은 오늘의 내가 되자", author: "익명" },
            { quote_text: "실패는 성공의 어머니다", author: "에디슨" },
            { quote_text: "꿈을 계속 간직하고 있으면 반드시 실현할 때가 온다", author: "괴테" },
            { quote_text: "시작이 반이다", author: "아리스토텔레스" },
          ];
          
          const randomQuote = fallbackQuotes[Math.floor(Math.random() * fallbackQuotes.length)];
          setDailyQuote(randomQuote.quote_text);
          setQuoteAuthor(randomQuote.author);
        }
      } catch (error) {
        console.error("명언 처리 중 전체 오류:", error);
        
        // 모든 처리가 실패한 경우 기본 메시지
        setDailyQuote("오늘도 화이팅!");
        setQuoteAuthor("");
      } finally {
...
<<<<<<< HEAD
                {currentUser?.user_consecutive_days ?? attendanceCount ?? 0}일 연속 출석 중
=======
...
<<<<<<< HEAD
            <h3 className="popup-title">{attendanceCount ?? currentUser?.user_consecutive_days ?? 1}일 연속 출석!</h3>
=======
